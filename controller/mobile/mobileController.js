...
const register=async(req,res,next)=>{
    ...
    try{
        ...
        let createcustomer=await DB.userrAccount.create({phoneNumber:phoneNumber});
...
const removeBook=async(req,res,next)=>{
    ...
    try{
        ...
        //
        let tripToRemove=await DB.customer.findOne({
         where:{id:customerId},
         include:{
            model:DB.trip,
            where:{
        ...
        if(!tripToRemove){
            const err=new Error('this trip not found or its not available yet');
            err.statusCode=422;
            throw err;
        }
        let checkLast12Houres=((new Date()).getTime())+12*60*60*1000;
        if(tripToRemove.trips[0].date<=checkLast12Houres){
        ...
        //remove reservation
        let chearArray=tripToRemove.trips[0].reservation.chearNum.split(',');

         let chear=await DB.chearr.findAll({where:{number:chearArray}});

        await tripToRemove.trips[0].addChears(chear,{through:{isBooked:'no',bookingDate:null}});
        let customerToRemove=await tripToRemove.trips[0].getCustomers({through:{
            model:DB.reservation,
            where:{chearNum:tripToRemove.trips[0].reservation.chearNum}
        }});
...
const rateTrip=async(req,res,next)=>{
    ...
    try{
        ...
        //
        let custm=await DB.customer.findOne({
            where:{id:customerId},
            include:{model:DB.trip,where:{id:tripId},attributes:['id']}
        });
        if(!custm){
        ...
        await custm.addTrips(custm.trips[0],{through:{rate:rate,comment:comment}});
        res.status(201).json({message:"thanks for your collaporations"});
    }catch(err){
        if(!err.statusCode)
               err.statusCode=500;
        next(err);
